...
def test_assistant_tool_call_and_tool_reply():
    assistant = Message(
        ...
        tool_calls=[ToolCall(id="call-1", name="http_request", arguments={"url": "https://api"})],
    )
    tool = Message(
        role="tool",
        content="OK",
    ...
    rec = ConversationRecord(messages=[Message(role="user", content="status?"), assistant, tool])
